...
export default function CreateLicensePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccess({ licenseKey: data.license.licenseKey });
    } catch {
    ...
    return (
      <div className="animate-fade-in flex items-center justify-center min-h-[60vh]">
        <div className="glass-card w-full max-w-md p-8 text-center">
          ...
          <p className="mt-2 text-sm text-white/40">Copy the license key below</p>
          <div className="mt-5 rounded-xl border border-white/[0.08] bg-white/[0.04] p-4">
            <code className="font-mono text-sm text-[#5f83f4] break-all select-all">
              {success.licenseKey}
            </code>
          </div>
          <div className="mt-6 flex gap-3">
            ...
            <Link href="/licenses" className="btn-primary flex-1">
              View All
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="mb-8 pt-8 lg:pt-0">
        ...
        <p className="mt-1 text-sm text-white/40">Generate a new license key for a domain</p>
      </div>
      ...
      <div className="glass-card w-full max-w-lg p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            />
            <p className="mt-1.5 text-[11px] text-white/20">
              Optional — associates license with a client
            </p>
          </div>

          <div>
            <label className="mb-3 block text-xs font-semibold uppercase tracking-wider text-white/30">
              ...
              {(["basic", "pro"] as const).map((p) => (
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition-all duration-200 ${
                    plan === p
                      ? "border-[#3b5eee]/50 bg-[#3b5eee]/10 shadow-lg shadow-blue-500/10"
                      : "border-white/[0.06] bg-white/[0.02] hover:border-white/[0.12]"
                  }`}
                >
                  <p className={`text-sm font-semibold uppercase ${plan === p ? "text-[#5f83f4]" : "text-white/50"}`}>
                    {p}
                  </p>
                  <p className="mt-0.5 text-[11px] text-white/25">
                    {p === "basic" ? "Core features" : "All features"}
                  </p>
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {(["1m", "3m", "6m", "12m"] as const).map((d) => (
                <button
                  ...
                  className={`rounded-xl border px-3 py-2.5 text-center text-sm font-medium transition-all duration-200 ${
                    duration === d
                      ? "border-[#3b5eee]/50 bg-[#3b5eee]/10 text-[#5f83f4]"
                      : "border-white/[0.06] bg-white/[0.02] text-white/40 hover:border-white/[0.12]"
                  }`}
                >
                  {d === "1m" ? "1 Mo" : d === "3m" ? "3 Mo" : d === "6m" ? "6 Mo" : "1 Yr"}
                </button>
              ))}
            </div>
          </div>
